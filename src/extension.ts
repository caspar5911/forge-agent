...
async function runForge(
  ...
  if (gitActions.length > 0) {
    ...
    await runGitActions(gitActions, rootPath, output, panelApi);
    setStatus('Done');
    return;
  }
...
async function runGitStage(rootPath: string, output: vscode.OutputChannel): Promise<void> {
...
async function runGitCommit(rootPath: string, output: vscode.OutputChannel): Promise<void> {
  output.appendLine(`[git] root: ${rootPath}`);
  if (!(await isGitRepo(rootPath))) {
    output.appendLine('[git] Not a repository (missing .git).');
    void vscode.window.showInformationMessage('Forge: Not a Git repository.');
...
async function runGitPush(rootPath: string, output: vscode.OutputChannel): Promise<void> {
  output.appendLine(`[git] root: ${rootPath}`);
  if (!(await isGitRepo(rootPath))) {
    output.appendLine('[git] Not a repository (missing .git).');
    void vscode.window.showInformationMessage('Forge: Not a Git repository.');
...
function detectGitActions(instruction: string): GitAction[] {
  const text = instruction.toLowerCase();
  const hasGitContext = /\b(git|repo|repository|branch|origin|remote|commit|push|stage|pull|diff|fetch|checkout|switch|status|log|history|remotes|sync|update|stash|unstage)\b/.test(text);
  ...
  const wantsStashPop =
    /\bstash\s+pop\b/.test(text) ||
    /\bpop\s+stash\b/.test(text);
  const wantsStashApply =
    /\bstash\s+apply\b/.test(text) ||
    /\bapply\s+stash\b/.test(text);
  const wantsStash =
    /\bstash\b/.test(text) && !(wantsStashList || wantsStashPop || wantsStashApply);
...
async function runGitActions(
  ...
  for (const action of actions) {
    ...
    if (action.type === 'stash-apply' || action.type === 'stash-pop') {
      ...
      const result = action.type === 'stash-pop'
